...
interface JvmProps extends ConnectProps {
  loading: boolean;
}

interface JvmState {
  parameter:  any
  jvmTitlecolumns:Array<Object>,
}

class Jvm extends React.Component<JvmProps,JvmState> {
  state:JvmState = {
    parameter: {
      keyword:''
    },
  ...
  render() {
    this.props.menuList.returnObject = [
      {
        "parameter": "jvm.memory.max",
        "description": "	JVM 最大内存",
        "id": 1,
        "current": "5197.500 MB",
      },
      {
        "parameter": "jvm.buffer.memory.used",
      ...
      <PageHeaderWrapper>
        <div className={styles.jvmPage}>
          ...
          <div className={`mT20 border ${styles.pageContent}`}>
            <Table<any>  locale={{filterConfirm: '确定',filterReset: '重置'}} columns={this.state.jvmTitlecolumns} size="middle"  rowKey={record => record.id} dataSource={this.props.menuList.returnObject} pagination={false} />
          </div>
          <div className="mB10" >
